...
resource "aws_sqs_queue_policy" "qida_emails_sqs_queue_policy" {
  queue_url = aws_sqs_queue.qida_emails_sqs_queue.id
    ...
    Statement = [
      {
        ...
        Resource = aws_sqs_queue.qida_emails_sqs_queue.arn
        Condition = {
          ArnEquals = {
            "aws:SourceArn" = var.sns_topic_emails_arn
          }
        }
      }
    ]
