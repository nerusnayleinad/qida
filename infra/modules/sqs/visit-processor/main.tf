...
resource "aws_sqs_queue" "qida_visits_sqs_queue" {
  ...
  redrive_policy = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.qida_visits_sqs_dlq.arn
    maxReceiveCount     = 3
  })
}

resource "aws_sqs_queue" "qida_visits_sqs_dlq" {
  name              = "sqs-dlq-visits-${var.name_suffix}"
  kms_master_key_id = var.kms_key_id
}
...
# CloudWatch
resource "aws_cloudwatch_metric_alarm" "visits_dlq_alarm" {
  alarm_name          = "visit-processor-dlq-messages"
  ...
  dimensions = {
    QueueName = aws_sqs_queue.qida_visits_sqs_dlq.name
  }
